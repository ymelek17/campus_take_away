        ...
        var body: some View {
          NavigationView {
            ...
            Group {
                ...
                //READY
                VStack {
                    NavigationLink(destination: UserMainNavigator().navigationBarBackButtonHidden(true).environmentObject(basketInfo), isActive: $isShowingMainView) {  }
                    Button(action: {
                     // ACTION
                     if
                         self.password == "" ||
                         self.email == ""
                     {
                        ...
                        requests.myGroup.notify(queue: DispatchQueue.main, execute: {
                            ...
                            requests.getAllCafes()
                            requests.myGroup1.notify(queue: DispatchQueue.main, execute: {
                                    for (key,value) in cafeList {
                                        basketInfo.timeslots_cafes[key] = value.timeslot_keys
                                    }
                                print(basketInfo.timeslots_cafes)
